...
import com.example.trainstationtemp.dto.PassengerDTO;
import com.example.trainstationtemp.repository.PassengerRepository;
...
public class PassengerController {
    ...
    @GetMapping
    @PreAuthorize("hasAnyAuthority('READ_PASSENGER')")
    public List<PassengerDTO> get(@RequestParam(name = "page") Integer pageId) {
    ...
    @PostMapping
    @PreAuthorize("hasAnyAuthority('WRITE_PASSENGER')")
    public PassengerDTO post(@RequestBody PassengerDTO dto) {
        return mapper.mapToDto(repository.save(mapper.mapToEntity(dto)));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('WRITE_PASSENGER')")
    public PassengerDTO put(@PathVariable Long id, @RequestBody PassengerDTO dto) {
